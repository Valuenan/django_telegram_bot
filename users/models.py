...
class Profile(models.Model):
    ...
    is_admin = models.BooleanField(verbose_name='Работник мгазина', default=False)
    ...
    def __str__(self):
        return f'{self.user} - номер чата {self.chat_id} - работник {self.is_admin}'
...
class Carts(models.Model):
    ...
    def __str__(self):
        return f'{self.user} - товар {self.product} - работник {self.amount} - цена {self.product}'
...
class Orders(models.Model):
    ...
    admin_check = models.CharField(max_length=100, verbose_name='Заявка принята работником:')

    def __str__(self):
        return f'Заказ номер: {self.id} - {self.user} - сумма {self.order_price} - подтвердил {self.is_admin}, пометка удаления {self.soft_delete}'
